...
A function to assemble the global load vector $S\textsubscript{G}$ from the local member forces $S\textsubscript{L}$ by rotating them to global coordinates and the node loads by simply adding them to the vector is also part of the module.
...
\begin{equation} \label{a}
    a = \begin{cases}
            4              & \text{if } \epsilon \leq 0\\[1.2em]
            \dfrac{\epsilon * (sin(\epsilon) - \epsilon * cos(\epsilon))}{2 * (1 - cos(\epsilon)) - (\epsilon * sin(\epsilon))}               & \text{if } N > 0          \\[1.2em]
            \dfrac{\epsilon * (sinh(\epsilon) - \epsilon * cosh(\epsilon))}{2 * ((cosh(\epsilon) - 1) - \epsilon * sinh(\epsilon))}               & \text{if } N \leq 0
        \end{cases}
\end{equation}

\begin{equation} \label{b}
    b = \begin{cases}
            2              & \text{if } \epsilon \leq 0\\[1.2em]
            \dfrac{\epsilon * (\epsilon - sin(\epsilon))}{2*(1 - cos(\epsilon)) - \epsilon * sin(\epsilon) }               & \text{if } N > 0          \\[1.2em]
            \dfrac{\epsilon * (\epsilon - sinh(\epsilon))}{2*(cosh(\epsilon) - 1) - \epsilon * sinh(\epsilon)}               & \text{if } N \leq 0
        \end{cases}
\end{equation}

\begin{equation} \label{c}
    c = \begin{cases}
            3              & \text{if } \epsilon \leq 0\\[1.2em]
            \dfrac{\epsilon^2 * sin(\epsilon)}{sin(\epsilon) - \epsilon * cos(\epsilon)}               & \text{if } N > 0          \\[1.2em]
            \dfrac{\epsilon^2 * sinh(\epsilon)}{\epsilon * cosh(\epsilon) - sinh(\epsilon)}               & \text{if } N \leq 0
        \end{cases}
\end{equation}
